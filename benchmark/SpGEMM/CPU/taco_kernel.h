...
// taco "A(i,j)=B(i,k)*C(k,j)" -f=A:ds:0,1 -f=B:ds:0,1 -f=C:ds:0,1
// -write-source=taco_kernel.c -write-compute=taco_compute.c
// -write-assembly=taco_assembly.c
...
int pack_B(taco_tensor_t *B, int *B_COO1_pos, int *B_COO1_crd, int *B_COO2_crd,
           double *B_COO_vals) {
  int B1_dimension = (int)(B->dimensions[0]);
  int *restrict B2_pos = (int *)(B->indices[1][0]);
  int *restrict B2_crd = (int *)(B->indices[1][1]);
  ...
  while (iB_COO < pB_COO1_end) {
    ...
    while (kB_COO < B_COO1_segend) {
      ...
      if (B2_crd_size <= kB) {
        B2_crd =
            (int32_t *)realloc(B2_crd, sizeof(int32_t) * (B2_crd_size * 2));
        B2_crd_size *= 2;
      }
      B2_crd[kB] = k;
      kB++;
...
int pack_C(taco_tensor_t *C, int *C_COO1_pos, int *C_COO1_crd, int *C_COO2_crd,
           double *C_COO_vals) {
  int C1_dimension = (int)(C->dimensions[0]);
  int *restrict C2_pos = (int *)(C->indices[1][0]);
  int *restrict C2_crd = (int *)(C->indices[1][1]);
  ...
  while (kC_COO < pC_COO1_end) {
    ...
    while (jC_COO < C_COO1_segend) {
      ...
      if (C2_crd_size <= jC) {
        C2_crd =
            (int32_t *)realloc(C2_crd, sizeof(int32_t) * (C2_crd_size * 2));
        C2_crd_size *= 2;
      }
      C2_crd[jC] = j;
      jC++;
...
int unpack(int **A_COO1_pos_ptr, int **A_COO1_crd_ptr, int **A_COO2_crd_ptr,
           double **A_COO_vals_ptr, taco_tensor_t *A) {
  int *A_COO1_pos;
  int *A_COO1_crd;
  int *A_COO2_crd;
  double *A_COO_vals;
  ...
  for (int32_t i = 0; i < A1_dimension; i++) {
    for (int32_t jA = A2_pos[i]; jA < A2_pos[(i + 1)]; jA++) {
      ...
      if (A_COO_capacity <= jA_COO) {
        A_COO_vals = (double *)realloc(A_COO_vals,
                                       sizeof(double) * (A_COO_capacity * 2));
        A_COO_capacity *= 2;
      }
      A_COO_vals[jA_COO] = A_vals[jA];
      if (A_COO2_crd_size <= jA_COO) {
        int32_t A_COO2_crd_new_size =
            TACO_MAX(A_COO2_crd_size * 2, (jA_COO + 1));
        A_COO2_crd = (int32_t *)realloc(A_COO2_crd,
                                        sizeof(int32_t) * A_COO2_crd_new_size);
        A_COO2_crd_size = A_COO2_crd_new_size;
      }
      A_COO2_crd[jA_COO] = j;
      if (A_COO1_crd_size <= jA_COO) {
        A_COO1_crd = (int32_t *)realloc(A_COO1_crd, sizeof(int32_t) *
                                                        (A_COO1_crd_size * 2));
        A_COO1_crd_size *= 2;
      }
      A_COO1_crd[jA_COO] = i;
