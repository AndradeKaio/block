...
KERNEL_DISPLAY = {
    "prisma_gpu_cuda_fp64":            "BLOCKS GPU (fp64)",
    "prisma_gpu_cuda_fp64_row_group":  "BLOCKS GPU (fp64, row-group)",
    "prisma_gpu_cuda_fp32":            "BLOCKS GPU (fp32)",
...
def make_figure(stats_total: pd.DataFrame, stats_compute: pd.DataFrame) -> go.Figure:
    ...
    fig.update_layout(
        title=dict(
            text="SuiteSparse GPU SpMM benchmark",
            font=dict(size=17),
            x=0.5,
        ),
        legend=dict(
...
def main():
    ...
    fig = make_figure(stats_total, stats_compute)
