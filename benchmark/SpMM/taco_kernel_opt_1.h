...
// taco "A(i,j)=B(i,k)*C(k,j)" -f=A:dd:0,1 -f=B:ds:0,1 -f=C:dd:0,1 -s="reorder(i,k,j)" -s="precompute(B(i,k)*C(k,j),j,j)" -s="assemble(A,Insert)" -s="split(i,i0,i1,32)" -s="unroll(i,8)" -s="parallelize(i0,CPUThread,NoRaces)" -write-source=taco_kernel.c -write-compute=taco_compute.c -write-assembly=taco_assembly.c
