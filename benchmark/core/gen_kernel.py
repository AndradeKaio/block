#!/usr/bin/env python3
"""
gen_kernel.py — Generate named-register AVX-512/AVX2 specializations of
gemm_fixed<M,K,N,double> and the DISPATCH() include file.

Shared by SpGEMM, SpMM, and SpMV: all three want the same thing from a
specialized CPU kernel -- a compile-time-known (M, K, N) dense micro-GEMM
(C[M,N] += A[M,K] @ B[K,N]), register-tiled over M and vectorized over N for
the target ISA. What differs between them is only which shapes to ask for
and how to dispatch to them (exact-match for SpGEMM/SpMV; an N-chunking
wrapper around a menu of NR values for SpMM) -- both stay in each domain's
own driver code, not here. This script only knows about (M, K, N) triples.

Double precision throughout.

Two outputs (always written together, same directory):
  kernels_generated.hpp  — template specializations with named registers
  dispatch_generated.hpp — DISPATCH(m, k, n) lines

Usage:
    python3 gen_kernel.py --shapes 5x4x4,1x2x4 --out-dir /tmp/out

    # Print register budget per shape and exit:
    python3 gen_kernel.py --shapes 5x4x4,1x2x4 --verify
"""
...
def _isa_for_n(n: int) -> tuple[str, int]:
    ...
    width at all ("scalar") -- this is exactly the shape SpMV wants (a
    single output value per row), and needs zero special-casing here: it
    degenerates correctly through _gen_kernel's NV==0 path below.
...
def _gen_kernel(M: int, K: int, N: int, s: dict) -> list[str]:
    ...
    ISA with lanes=1 used for N==1 -- SpMV's shape) this degenerates to a
    pure scalar kernel: no SIMD registers/intrinsics are emitted at all.
    ...
    This same row-tiling is also what lets one N value serve every ISA: a
    caller asking for a wide N under AVX-512's 32-register budget and then
    falling back to AVX2's 16-register budget at the SAME N doesn't need a
    smaller N for the narrower ISA -- rows_per_tile shrinks automatically to
    fit whichever total_regs it's given.
    """
    lanes = s["lanes"]
...
def _emit_shape(M: int, K: int, N: int) -> list[str]:
    ...
    Every shape gets a real generated kernel -- N==1 (SpMV) works exactly
    the same as any other N, just degenerating to pure scalar; N not
    divisible by the chosen lane width still gets a real kernel via the
    fused scalar tail in _gen_kernel, not a dropped shape.
    """
    lines = []
...
def main():
    ...
    ap.add_argument(
        "--shapes",
        required=True,
        help="Comma-separated MxKxN shapes (e.g. '5x4x4,1x2x4'). Always three "
             "numbers -- SpGEMM passes mined (M,K,N) triples, SpMM passes "
             "(H,W,NR), SpMV passes (H,W,1); this script doesn't know or "
             "care which domain is calling it.",
    )
    ap.add_argument(
    ...
    kernels_path = out_dir / "kernels_generated.hpp"
    dispatch_path = out_dir / "dispatch_generated.hpp"

    # --- kernels_generated.hpp ---
    kern_lines = [
        "// AUTO-GENERATED by gen_kernel.py — do not edit.",
    ...
    # --- dispatch_generated.hpp ---
    dispatch_lines = [
        "// AUTO-GENERATED by gen_kernel.py — do not edit.",
        "// Included inside a caller's DISPATCH(m, k, n) macro scope.",
    ]
    for M, K, N in shapes:
